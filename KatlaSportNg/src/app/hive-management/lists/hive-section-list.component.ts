...
export class HiveSectionListComponent implements OnInit {
  ...
  onDelete(hiveSectionId: number) {
    ...
    this.hiveSectionService.setHiveSectionStatus(hiveSectionId, true).subscribe(hs => hiveSection.isDeleted = true);
  }

  onRestore(hiveSectionId: number) {
    var hiveSection = this.hiveSections.find(hs => hs.id == hiveSectionId);
    this.hiveSectionService.setHiveSectionStatus(hiveSectionId, false).subscribe(hs => hiveSection.isDeleted = false);
  }
}
